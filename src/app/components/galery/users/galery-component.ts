...
export class UsersGaleryComponent implements OnInit {
  @Input() user!: User;
  ...
  public async uploadData(formData: FormData): Promise<Subscription> {
    ...
    this.usersService.setCsrf = this.getcsrf();
  ...
  public destroyAvatar(): Subscription {
    return (this.destroy = this.avatarService
      // eslint-disable-next-line no-underscore-dangle
      .avatarUsersDestroy(this.avatar)
      .subscribe(
        () => {
          this.emptyImage();
          this.isAvatar();
  ...
  private sendFile(
    ...
    if (this.photoService.isPageUser()) {
      return (this.upload = this.usersService
  ...
  private update(avatar: Image, subscription: Subscription): void {
    this.helpsService.delay(() => {
      ...
      this.stopLoading();
      this.isAvatar();
      this.usersService.avatar = this.user?.image;
    }, 2500);
  ...
  private setAvatar(avatar: Image): void {
    const { filename, url } = this.avatarService.setUsersAvatar(avatar);
