...
export class PhotoService {
  ...
  public async deleteFile(file: LocalFile): Promise<void> {
    return await Filesystem.deleteFile(this.deleteFileOptions(file));
  }
  ...
  private async loadFileData(fileNames: FileInfo[]): Promise<void> {
    for (const f of fileNames) {
      const filePath = `${this.imageDir}/${f.name}`;
      const readFile = await Filesystem.readFile(
        this.readFileOptions(filePath)
      );
      PhotoService.images.push(this.addImages(f.name, filePath, readFile));
    }
  }
