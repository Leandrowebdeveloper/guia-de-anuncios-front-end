...
@Component({
  ...
  templateUrl: 'category-advert.page.html',
  styleUrls: ['category-advert.page.scss', '../users/users.page.scss'],
})
export class CategoryPage implements OnInit, OnDestroy {
  ...
  public saveOrder(): void {
    const loading = this.categoryService.showLoading('Ordenar categoria...');
    ...
    if (result.length > 0) {
      ...
      this.$order = this.categoryService.order(category).subscribe(
        (category_: Category) => {
          ...
          this.categoryService.message(category_, loading, this.$order);
        },
        (error: HttpErrorResponse) =>
          this.categoryService.error(error, loading, this.$order)
      );
    }
  }
